...
```{r set checkpoint, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
# ignore for the moment
# checkpoint("2019-09-20", checkpointLocation = "C:/devsbb/R/checkpoint")  # not working properly @Ramon, check with Tanadini
# checkpointRemove("2019-09-20", checkpointLocation = "C:/devsbb/R/checkpoint")
# .libPaths()
```
...
create_report(df)  # not working @Ramon, works @Lars & @Carmela
```
...
df_scaled_df$Col_qual <- rbPal(10)[as.numeric(cut(df_scaled_df$quality,breaks = 10))]
df_scaled_df$Col_ph <- rbPal(10)[as.numeric(cut((df_scaled_df$ph),breaks = 10))]
...
plot(tsne$Y, col=df_scaled_df$Col, pch=16, main = "TSNE coloured by ph")
#legend("topleft", legend=levels(df$quality), pch=16, col=unique(df$quality))
