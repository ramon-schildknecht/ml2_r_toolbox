...
>>>>>>> c216d2ff096e9a59e112e6360fa65339d581f011
...
* Could not find any meaningful patterns
* I tried visually colouring the plotted datapoints by pH,citric.acid and quality
* The visual categorisation does not seem to work for these values
...
round(pca_out$rotation, digit=2) # round rotation (PC loadings) on 2 digits
...
* PC1 mostly represents quality, alcohol content and sulfur content of a wine.
* PC2 mostly represents how acidic or basic (alkaline) a wine is.
* alcohol and qulity are positiv correlated
* quality and sulfur content is negatively correlated
* fixed.acitiy and pH value point in the opposite direction (negatively correlated). pH value is a metric to measure the acidity. A low pH value indicates high acidity whereas a high pH value indicate a low acidity (alkaline). So the result in the biplot is conclusive.
```{r}
...
Those plots are used to check the plausibility of the biplot. The correlations discovered in the biplot can also be observed by the correlation plots below.
```{r}
